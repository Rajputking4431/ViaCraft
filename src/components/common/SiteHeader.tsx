...
import { getCartItemCount } from "@/features/cart/api/cart";
import { getWishlistCount } from "@/features/wishlist/api/wishlist";
                      ...
                      <div className="flex flex-wrap gap-2">
                        {POPULAR_SEARCHES.map((term) => (
                          <button
                            key={term}
                            onClick={() => handleSearchSubmit(term)}
                            className="px-3.5 py-1.5 rounded-full border border-border/70 bg-card hover:bg-accent/5 hover:border-accent text-xs text-foreground/85 hover:text-accent transition-all cursor-pointer font-medium"
                          >
                            {term}
                          </button>
                        ))}
          ...
          <div className="mx-auto max-w-7xl flex items-center justify-between">
            ...
            <div className="relative shrink-0">
              <button
                ...
                className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-foreground/85 hover:text-accent py-2 px-4.5 rounded-full border border-border bg-background cursor-pointer transition-all hover:shadow-sm"
              >
