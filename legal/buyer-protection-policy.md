...
```mermaid
graph TD
    A[Issue Occurs] --> B[Customer contacts Vendor via Dashboard]
    B --> C{Resolved in 48 Hours?}
    C -- Yes --> D[Order Closed]
    C -- No --> E[Customer Escalates to Support Email]
    E --> F[Platform Team Reviews Evidence]
    F --> G{Valid Claim?}
    G -- Yes --> H[Platform processes Refund / Replacement]
    G -- No --> I[Dispute Closed in Vendor favor]
```
